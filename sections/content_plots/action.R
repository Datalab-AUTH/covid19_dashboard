...
output$action_cases <- renderPlotly({
  ...
  req(input$action_taken)
  data <- inner_join(data_oxford, data_confirmed_1st_case, by = "iso3c") %>% 
    arrange(iso3c, ActionDate) %>% 
    rename("ConfirmedDate" = "date") %>% 
    group_by(iso3c)
  if (input$action_taken == "S1_School closing") {
    data <- data%>% 
      filter(`S1_School closing` > 0 ) %>%
      slice(1)
  } else if (input$action_taken == "S2_Workplace closing") {
    data <- data%>%
      filter(`S2_Workplace closing` > 0 ) %>%
      slice(1)
  } else if (input$action_taken == "S3_Cancel public events") {
    data <- data%>%
      filter(`S3_Cancel public events` > 0 ) %>%
      slice(1)
  } else if (input$action_taken == "S4_Close public transport") {
    data <- data%>%
      filter(`S4_Close public transport` > 0 ) %>%
      slice(1)
  } else if (input$action_taken == "S5_Public information campaigns") {
    data <- data%>%
      filter(`S5_Public information campaigns` > 0 ) %>%
      slice(1)
  } else if (input$action_taken == "S6_Restrictions in internal movement") {
    data <- data%>%
      filter(`S6_Restrictions in internal movement` > 0 ) %>%
      slice(1)
  } else if (input$action_taken == "S7_International travel controls") {
    data <- data%>%
      filter(`S7_International travel controls` > 0 ) %>%
      slice(1)
  } else if (input$action_taken == "S8_Fiscal measures") {
    data <- data%>%
      filter(`S8_Fiscal measures` > 0 ) %>%
      slice(1)
  } else if (input$action_taken == "S9_Monetary measures") {
    data <- data%>%
      filter(`S9_Monetary measures` > 0 ) %>%
      slice(1)
  } else if (input$action_taken == "S10_Emergency investment in health care") {
    data <- data%>%
  ...
  p <- plot_ly(
    ...
    layout(
      ...
      xaxis = list(title = paste("Delay of", input$action_taken, "since first confirmed case")),
      showlegend = TRUE
    )
...
output$select_action_variable <- renderUI({
  actions <- data_oxford %>% 
...
output$box_case_action <- renderUI({
  tagList(
    fluidRow(
      box(
        title = "Actions vs first confirmed case",
        ...
        column(
          uiOutput("select_action_variable"),
          width = 4
        ),
        column(
