...
output$oecd_expectancy <- renderPlotly({
  req(input$case_oecd_expectancy_var)
  req(input$case_oecd_expectancy_limit)
  
  data_conf <- data_latest %>%
    select("Country/Region", "population", input$case_oecd_expectancy_var) %>%
    rename("Country" = "Country/Region") %>%
    rename("values" = input$case_oecd_expectancy_var) %>%
    group_by(Country) %>%
    summarise(
      "population" = sum(population, na.rm = T),
      "values" = sum(values, na.rm = T)
    ) %>%
    filter(values >= as.numeric(input$case_oecd_expectancy_limit)) %>%
    mutate(iso3c = countrycode(Country, origin = "country.name", destination = "iso3c"))
  
  if (input$case_oecd_expectancy_var == "confirmed") {
    y_label = "# Confirmed Cases"
    dot_color = "#FC8D62"
  } else if (input$case_oecd_expectancy_var == "recovered") {
    y_label = "# Recovered"
    dot_color = "E78AC3"
  } else if (input$case_oecd_expectancy_var == "deceased") {
    y_label = "# Deceased"
    dot_color = "#8DA0CB"
  } else { # active cases
    y_label = "# Active Cases"
  ...
  correlation <- cor.test(data_merged$values, data_merged$lifeExpectancy, method = "spearman")
  p_value <- correlation$p.value
  rho <- round(correlation$estimate, 3)
  if (p_value < 0.001) p_value_text = "P < 0.001"
  else if (p_value < 0.01) p_value_text = "P < 0.01"
...
output$select_cases_oecd_expectancy_variable <- renderUI({
  selectizeInput(
    "case_oecd_expectancy_var",
    label    = "Select Variable",
    choices  = list("Confirmed" = "confirmed", "Recovered" = "recovered", "Deceased" = "deceased", "Active" = "active"),
    multiple = FALSE
  )
...
output$select_cases_oecd_expectancy_limit <- renderUI((
  selectizeInput(
    "case_oecd_expectancy_limit",
    label = "Number of cases",
    choices = list("≥1" = 1, "≥10" = 10, "≥100" = 100, "≥1,000" = 1000, "≥10,000" = 10000),
    multiple = FALSE
  )
