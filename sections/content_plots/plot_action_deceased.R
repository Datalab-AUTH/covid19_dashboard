output$action_deaths <- renderPlotly({
  ...
  data <- data %>%
    ...
    mutate(Country = countrycode(iso3c, origin = "iso3c", destination = "country.name"),
           ...
           diff = as.integer(ActionDate - ConfirmedDate))
  ...
  for (country in input$action_deaths_country) {
...
output$select_action_deaths_country_variable <- renderUI({
